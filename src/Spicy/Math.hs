...
ltMat2Square ltVec = do
  ...
  rootExpr <- case exactSquareRoot (8 * nElemLT + 1) of
    Just x -> return x
    Nothing ->
      throwM $ DataStructureException "ltMat2Square" $ "Cannot take the square root of " <> show
        (8 * nElemLT + 1)
 ...
 where
  ...
  lineariseIndex (ixR, ixC) = if ixR >= ixC
-- We are in the lower triangular matrix part (including the main diagonal).
    then RIO.sum [0 .. ixR] + ixC

-- We are in the upper triangular part (excluding the main diagonal) of the square matrix.
    else RIO.sum [0 .. ixC] + ixR
