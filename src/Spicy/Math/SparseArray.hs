...
-}
data SparseArray i a = SparseArray
  !(i, i)        -- ^ Lower and upper bound of the array.
  !(HashMap i a) -- ^ The "non empty" values of a sparse array, which are given.
  !a             -- ^ The "empty" value, which all other elements implicitly have.
...
data IndexException = IndexException
  { indExcFunctionName :: String -- ^ Function which is causing the exception.
  , indExcDescription  :: String -- ^ Description of the problem.
  }
...
sparseArray
  ...
  => (i, i)               -- ^ Lower and upper bound of the index range.
  -> e                    -- ^ Default element. Will not appear in the map to save memory.
  -> f (i, e)             -- ^ An arbitary 'Foldable' structure with @(index, element)@ mappings.
  -> m (SparseArray i e)  -- ^ Resulting sparse array.
sparseArray ind@(indLow, indHigh) defElement assocs'
  | not orderCheck = throwM
...
foldableSparseArray indRange defElement values
  | not lengthCheck = throwM $ IndexException
    "foldableSparseArray"
...
arr@(SparseArray ind _gV _dV) `lookup` ix
  ...
  where boundaryCheck = inRange ind ix

unsafeLookUp :: (Ix i, Hashable i) => SparseArray i a -> i -> a
(SparseArray ind gV dV) `unsafeLookUp` ix | not boundaryCheck = error "Indices are out of bound."
...
indexReplacement repMap (SparseArray indRange gV dV) = do
  ...
  when (numberOfPossibleReplacements /= numberOfReplacementAttempts) $ throwM $ IndexException
    "indexReplacement1"
  ...
  unless allIndicesInRange $ throwM $ IndexException
    "indexReplacement1"
  ...
  let newGv = HashMap.foldlWithKey'
        (\accMap ix v ->
...
foldlWithIndex' f start (SparseArray indRange gV dV) =
  ...
  in  foldl' (\acc ix -> let valAtIx = HashMap.lookupDefault dV ix gV in f acc ix valAtIx)
             start
             allIndices
...
foldrWithIndex' f start (SparseArray indRange gV dV) =
  let allIndices = range indRange
  in  foldr (\ix acc -> let valAtIx = HashMap.lookupDefault dV ix gV in strictF ix valAtIx acc)
            start
            allIndices
  where strictF ix val !acc = f ix val acc
...
mapWithIndex
  ...
  => (i -> a -> b)    -- ^ Function, that takes an 'Ix' index and a value and constructs the new
                      --   value at this index.
  -> (a -> b)         -- ^ Function that updates the default value to a new one, independent from its
                      --   index.
  -> SparseArray i a  -- ^ Original array.
  -> SparseArray i b
...
traverseWithIndex
  ...
  => (i -> a -> f b)     -- ^ Function, that takes an 'Ix' index and a value and constructs the new
                         --   value at this index.
  -> (a -> f b)          -- ^ Function to update the default value, independent from its index.
  -> SparseArray i a     -- ^ Array to update.
  -> f (SparseArray i b)
...
isSymmetricMatrix arr@(SparseArray _ gV _) = isSquareMatrix arr && isSymmetric
 where
  isSymmetric = HashMap.foldlWithKey'
    (\accBool (ixR, ixC) el -> if ixR >= ixC
      then
...
homogenousIndexReplacement2 repMap arr@(SparseArray indRange@((lowBoundX, _), (highBoundX, _)) gV dV)
  = do
    ...
    unless (isSquareMatrix arr) $ throwM $ IndexException
    ...
    when (nRepKeys /= nRepValues) $ throwM $ IndexException
      "homogenousIndexReplacement"
      "Replacement targets occur multiple times and elements would be lost.\
      \ This replacement will not be performed."
