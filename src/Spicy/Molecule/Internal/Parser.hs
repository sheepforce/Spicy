...
parseTXYZ = do
 ...
 where
  ...
  txyzLineParser = do
    ...
    return TXYZAtomInfo { txyzIndex = index, txyzAtom = atom, txyzBondTargets = connectivityRaw }
...
parseMOL2 = do
  ...
  let
    -- Construct the atoms and fragments of the top layer from the atom block.
  ...
  atomParser nAtoms = do
    ...
    atomAndFragmentInfo <- count nAtoms $ do
      ...
      let mol2FFText =
          ...
          thisAtomsFragment = Fragment { _fragment_Label = subName
                                       , _fragment_Chain = Nothing
                                       , _fragment_Atoms = IntSet.singleton index
                                       }
      return FragmentAtomInfo { faiAtomIndex     = index
 ...
 where
  ...
  atomParser = do
    ...
    let atom = Atom
        ...
        fragment = Fragment { _fragment_Label = fragmentLabel
                            , _fragment_Chain = Just chainID
    ...
    return FragmentAtomInfo { faiAtomIndex     = atomKey
