...
-- | Given a vector of size \(3N\) with \(N\) being the number of atoms, update the position of all
-- atoms in a Molecule. This includes link atoms of deeper layers but excludes dummy atoms, which
-- will be removed.
updateMolWithPosVec :: MonadThrow m => Vector S Double -> Molecule -> m Molecule
updateMolWithPosVec pos mol = do
  let nAtomsReal = IntMap.size . IntMap.filter (not . isDummy) $ mol ^. #atoms
      dense2Sparse = getDenseSparseMapping mol

  -- Reconstruction of the position data into individual coordinates of atoms.
  nCoordSlices <-
    Massiv.map (VectorS . compute @S) . Massiv.outerSlices
      <$> Massiv.resizeM (Sz $ nAtomsReal :. 3) pos
  let newAtomCoords =
        ifoldMono
          ( \dIx coords ->
              let sIx = dense2Sparse Massiv.! dIx
               in IntMap.singleton sIx coords
          )
          nCoordSlices

  -- Update all atoms coordinates recursively while removing all dummy atoms.
  go newAtomCoords mol
  where
    localExc = MolLogicException "updateMolWithPosData"
    ...
    go :: MonadThrow m => IntMap (VectorS Double) -> Molecule -> m Molecule
    go newCoords molLayer = do
